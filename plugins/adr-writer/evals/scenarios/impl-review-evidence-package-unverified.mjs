...
export default {
  ...
  build(dir) {
    ...
    return [
      ...
      `Context fields must be exactly: Payment settlement preserves one durable result. / Payment settlement receives retries and provider outcomes. / One completion boundary preserves idempotency and pending state. / Duplicate settlement passed while provider-failure verification remains unavailable.`,
      ...
      `Hill H1 Container must explain retry responsibility, interactions, and one-completion outcome. Component C1 must explain the idempotent boundary and include focused diff Code evidence.`,
      `Hill H2 heading: Provider failure remains the important unknown.`,
      `Hill H2 vertical slice must be user-flow / Provider failure settlement.`,
      `Hill H2 Container must explain provider-failure responsibility, interactions, and unverified pending outcome. Component C1 must explain the failure branch and include focused diff Code evidence.`,
  ...
  score({ tail, output, dir }) {
    ...
    const artifact = validateReviewArtifact(dir, visible, {
      ...
      diagramRequirements: [
        ...
        diagramType,
        question,
        section: "Provider failure remains the important unknown",
      ...
      reviewHike: {
        context: {
          ...
          scopeAndRisk:
            "Duplicate settlement passed while provider-failure verification remains unavailable.",
        },
        hills: [
          {
            id: "H1",
          ...
          {
            id: "H2",
            title: "Provider failure remains the important unknown",
            sliceType: "user-flow",
            ...
            assessment: "The path remains unverified because failure injection is unavailable.",
            container: {
              responsibility: "Keep provider failure outside completion.",
              interactions: "The provider failure should return to settlement.",
              outcome: "The pending result remains unverified.",
            },
            components: [
              {
                id: "C1",
                ...
                implementation: "The failure path should keep the payment pending.",
                verification: "Provider-failure injection is unavailable.",
                codeEvidence: [
                  {
                    kind: "diff",
                    location: "src/payments/settle.ts:58",
                    content: "- recordCompletion(payment)\n+ keepPending(payment)",
                    explanation: "The branch intends to preserve pending state.",
      ...
      reviewDiagnostics: {
        ...
        testSufficiency: {
          status: "UNVERIFIED",
          assessment: "Provider failure lacks executed evidence.",
