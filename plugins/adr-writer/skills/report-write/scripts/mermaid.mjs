...
/** Prefer word boundaries; split only a token too long for the available label width. */
function wrap(value, capacity = 26) {
  const lines = [];
  const sizeOf = (word) =>
    [...word].reduce((sum, char) => sum + (/[^\x00-\x7f]/.test(char) ? 2 : 1), 0);
  for (const paragraph of String(value).split("\n")) {
    let line = "",
      width = 0;
    for (const word of paragraph.match(/\S+/g) || []) {
      if (line && width + 1 + sizeOf(word) > capacity) {
        lines.push(line);
        line = "";
        width = 0;
      }
...
export function renderMermaid(source, ui) {
  ...
  const prefix = ui.idPrefix ? `${String(ui.idPrefix).replace(/[^a-z0-9_-]/gi, "-")}-` : "";
  const id = `mermaid-${prefix}${createHash("sha256").update(source).digest("hex").slice(0, 12)}`;
  const typeClass = {
