...
import { quizPage } from "./quiz-browser-fixture.mjs";
...
test("comprehension questions render without exposing grading criteria", () => {
  ...
  const [quiz] = quizPage(result.stdout);
  const control = (name) => quiz.controls.get(`.quiz__${name}`);
  assert.equal(control("options").hidden, true);
  assert.equal(control("feedback").hidden, true);
  quiz.click("reveal");
  assert.equal(control("options").hidden, false);
  assert.equal(control("reveal").hidden, true);
  quiz.click("check");
  assert.equal(control("required").hidden, false);
  assert.equal(control("feedback").hidden, true);
  quiz.select("A");
  assert.equal(control("teach-back").hidden, false);
  assert.equal(control("feedback").hidden, true);
  quiz.click("check");
  assert.equal(control("result").textContent, "Review this concept");
  assert.equal(control("selected-feedback").textContent, "SECRET_FEEDBACK_A");
  quiz.select("B");
  assert.equal(control("feedback").hidden, true);
  quiz.click("check");
  assert.equal(control("result").textContent, "Correct");
  assert.equal(control("criteria").textContent, "SECRET_ANSWER_EXPLANATION");
  assert.equal(control("evidence").textContent, "SECRET_GRADING_EVIDENCE");
