...
| `/adr-impl-review [category]`    | Derive the complete implementation scope from every ADR decision and contract row instead of treating the diff as a ceiling, then select `standard` or `full` by risk. Both explain ADR intent and the major algorithm or flow, require grounded Mermaid for multi-step relationships, group findings into fix/decision/verification/suggestion tasks, and open the validated standalone HTML report once in the default browser. The ordinary main-session response never prints or grades the collapsed comprehension questions; interactive grading starts only by explicit user request (report-only) |
...
| `scripts/adr-impl-review-open.mjs`        | rendered HTML report                                   | Rejects a missing or empty report, then opens a valid report exactly once with the host's default browser; unavailable openers keep the verified path and report the reason.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
