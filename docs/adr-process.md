...
- **리포트는 처음 보는 주니어가 점진적으로 읽는다.** 두 모드 모두 검증된 `adr-impl-review-report.html`을 생성한다. HTML은 초록에서 결론과 다음 행동을 먼저 보여주며, 목차와 상세 근거는 필요할 때 펼친다. ADR 의도와 핵심 흐름, finding을 읽은 뒤 `PROVEN` coverage·scope·metrics·구현 선택을 확인할 수 있다. 각 Hill은 질문별 다이어그램을 배정하거나 단순한 국소 변경이라는 개별 생략 근거를 기록한다. 요청·응답은 시퀀스도, 책임·의존·경계는 구성도를 우선하며 상태도는 수명주기 자체가 핵심일 때 선택한다. 공통 구성도는 Context에서 관련 Hill로 연결한다. validator는 배정·참조·종류·위치와 렌더링 가능한 문법을 검사하고 실제 HTML에서도 관계와 가독성을 확인한다. 원문 fallback은 완료된 필수 시각화로 집계하지 않는다.
...
- **코드 PASS와 PR 이해 준비도는 다르다.** 사용자 요청이나 변경의 복잡성 때문에 이해도 확인이 필요할 때만, 중요한 동작과 인과관계를 묻는 중간 난이도 4지선다 단일정답 질문을 1~5개 만든다. HTML은 질문과 회상 안내를 먼저 보여주고 사용자가 요청한 뒤 선택지를 펼친다. 선택 후에는 입력·채점 없는 한 문장 설명 유도를 거쳐 self-check에서 판정 기준을 공개하며, 답을 바꾸면 이전 결과를 숨긴다. 핵심 질문 1~2개는 후속 재점검 대상으로 표시하지만 시간·알림·진행 상태를 저장하지 않는다. 사용자가 이해도 확인을 명시적으로 요청한 경우에만 질문을 하나씩 대화형으로 진행하며, 실패·미응답이면 근거를 설명하고 PR comprehension-ready라고 안내하지 않는다.
