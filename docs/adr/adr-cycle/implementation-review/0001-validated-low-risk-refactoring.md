...
대시보드형 리포트는 verdict, finding과 coverage를 빠르게 나열하지만 독자가 하나의 주장과 근거를 따라가며 구현을 이해하기 어렵다. 사람용 산출물은 전체 판단에서 도메인별 상세로 내려가며, 각 설명 단위는 주장, 근거와 평가를 가까이 배치해야 한다. 기계 감사 자료는 근거 부록에 보존하되 본문 독해를 방해하지 않아야 한다.
...
- 해당 구현을 처음 보는 주니어 개발자가 도메인별 읽기 흐름에서 결론, 영향, 주요 알고리즘과 처리 흐름, 변경의 배경·핵심 직관·코드 흐름과 남은 위험을 상세 증거보다 먼저 이해하고 다시 설명할 수 있어야 한다. 본문의 각 분석 단위는 주장, 근거와 평가를 함께 보여주고, 감사용 원본은 근거 부록에서 추적할 수 있어야 한다. 각 finding에서는 무엇이 필수 수정인지, 무엇을 결정하거나 검증해야 하는지, 어디를 바꾸고 어떤 결과가 나오면 완료인지 추가 해석 없이 알 수 있어야 한다. 중요한 이해를 확인하지 못한 상태는 코드 적합성 `PASS`와 구분해 PR 전달 전에 드러내야 하며, standard와 full은 같은 독립 실행형 HTML Evidence Package를 기본 브라우저에서 바로 보여줘야 한다. 단순한 국소 PASS에는 고정 형식 비용을 만들지 않아야 한다.
...
사람용 Evidence Package는 공통 보고서 계약에 따라 전체 판단에서 도메인별 상세로 내려가는 계층을 사용한다. 의도와 계약, 핵심 동작과 알고리즘, 검증 결과, finding과 한계, 다음 행동과 완전한 근거를 보존하되 평면적인 고정 목차로 중복하지 않는다. 구조화된 감사 원본은 그대로 유지하고 사람용 보기는 같은 수준의 직접 하위 설명 단위를 최대 4개로 구성한다.
...
사람용 구현 리뷰는 제목과 핵심 결과 다음에 확인된 도메인·바운디드 컨텍스트별로 책임, 동작, 검증과 판단을 연결한다. 파일·기술 계층·작업 순서를 도메인 경계로 사용하지 않는다. 이해도 확인은 관련 도메인의 판단 다음에 연결하고 필요한 원문 근거로 내려갈 수 있게 한다. 기존 기계용 원본의 필수 필드와 식별자는 보존한다.
...
사람용 HTML은 개요와 도메인별 계층, 구현과 근거의 확대 단계를 제공한다. 각 도메인 안에서 주장·근거·검증·판단을 연결하고 전체 coverage와 원본 evidence를 누락하지 않는다. PROVEN 근거는 접고 VIOLATED·UNVERIFIED·CONTRADICTED 근거는 펼친다. 판단이 필요한 finding에만 ruling을 요구한다. 필요하면 구조화된 감사용 표현과 최종 사람용 리포트를 분리하되 사용자에게는 공통 보고서 계약을 만족하는 최종 리포트를 제공한다.
...
- 사람용 리포트는 공통 보고서 계약에 따라 개요에서 도메인별 상세와 근거로 내려가며, 직접 하위 설명 단위는 최대 4개다. 기존 의미·계약·검증 결과를 삭제해 상한을 맞추지 않는다.
...
- 서로 다른 구현 주제의 fixture가 모두 제목과 전체 판단으로 시작하고, 도메인별 설명에서 contract coverage 전에 Container/Hill, Component와 Code 근거를 주장→근거→평가 순서로 제공한다.
...
- 이해도 확인은 관련 도메인의 판단 다음에 연결하고 질문을 기본으로 표시한다. 문항 수·선택지·정답 숨김 규칙을 유지하며 다섯 문항을 동급으로 나열하지 않도록 업무 개념으로 묶는다.
...
- 자유로운 주제별 heading이 보고서마다 일관성을 잃을 수 있다. 각 도메인 안에서 맥락, 동작, 판단과 근거의 관계를 명시하고 주제에 맞는 제목을 사용한다. 감사용 원본의 필수 anchor는 보존한다.
...
- [공통 리포트의 도메인 탐색](../../reporting/domain-navigation/0001-domain-first-report-navigation.md) — 최종 사람용 표현에 적용한다.
