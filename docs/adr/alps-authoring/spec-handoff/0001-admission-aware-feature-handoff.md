...
이전 범위의 이해에 필요한 용어 정의는 PRD의 선택적 용어 부록과 사용자 설명에서 찾아
ADR 루트의 `glossary.md`로 함께 이전한다. 필요한 정의가 없으면 빈 파일을 만들지 않는다.
같은 뜻은 재사용하고 관련 없는 기존 항목은 보존한다. 뜻이 충돌하거나 불명확하면 사용자에게
확인하기 전 해당 의미에 의존하는 인계를 완료하지 않는다. 정의는 기존 계약 승인에 포함한다.

이전된 용어집은 ADR 수준의 자료로 관리하며 PRD를 다시 읽어야 하는 참조나 출처 식별자를
저장하지 않는다. 상태·권한·값·실패 보장과 성공 조건은 용어집에만 남기지 않고 소유 ADR의
계약으로 이전한다. 명시적 재import에서도 같은 뜻은 변경하지 않고, 의미 변경이나 삭제는
현재 용어와 소유 ADR에 미치는 영향을 확인한 뒤 처리한다.
...
- 이전 범위에 필요한 확인된 정의는 ADR 루트의 용어집으로 이전하고, 대상 정의가 없으면
  새 용어집을 생성하지 않는다.
- 필요한 용어의 뜻이 불명확하거나 기존 정의와 충돌하면 자동 확정하지 않고 사용자에게
  확인하며 그 의미에 의존하는 인계의 완료를 보류한다.
- 같은 뜻의 반복 인계는 용어집도 변경하지 않으며 관련 없는 기존 항목은 보존한다.
- 이전된 용어는 PRD 참조 없이 이해할 수 있어야 하고 실제 요구사항은 소유 ADR 본문에 남긴다.
- 관찰 기준: 인계 후 PRD 없이 대상 용어의 의미와 계약을 읽을 수 있고, 반복 인계·부분 인계가
  중복 정의나 기존 정의 손실을 만들지 않는다.
